...
describe( 'getButtonLabels', () => {
	...
	// Cancel intent fires the mutation at the confirmation click and always shows
	// "Cancel subscription" — the flag does not change this label.
	// Under the flag, Remove intent uses intent-aware labels ("Continue removal" etc).
	test( 'Cancel intent always uses "Cancel subscription" regardless of flag', () => {
	...
	test( 'skipSurvey collapses Remove intent to the direct product-specific label', () => {
		expect(
			getButtonLabels( {
				purchase: makePurchaseForCategory( 'plan' ),
				intent: 'remove',
		...
		expect(
			getButtonLabels( {
				purchase: makePurchaseForCategory( 'marketplace', {
					product_type: 'marketplace_theme',
				} as Partial< Purchase > ),
				intent: 'remove',
				skipSurvey: true,
			} )
		).toEqual( { primary: 'Remove theme', secondary: 'Keep theme' } );
		expect(
			getButtonLabels( {
				purchase: makePurchaseForCategory( 'marketplace' ),
				intent: 'remove',
		...
		).toEqual( { primary: 'Remove plugin', secondary: 'Keep plugin' } );
	} );
} );
