...
export function getButtonLabels( { purchase, intent, skipSurvey }: ConfirmationCopyArgs ): {
	...
	// button is the action — use the product-specific label rather than
	...
	if ( intent === 'remove' ) {
		switch ( category ) {
			case 'plan':
				return {
					primary: showContinueRemoval ? __( 'Continue removal' ) : __( 'Remove plan' ),
					secondary: __( 'Keep plan' ),
				};
			case 'domain':
				return {
					primary: showContinueRemoval ? __( 'Continue removal' ) : __( 'Remove domain' ),
					secondary: __( 'Keep domain' ),
				};
			case 'email':
				return {
					primary: showContinueRemoval ? __( 'Continue removal' ) : __( 'Remove email' ),
					secondary: __( 'Keep email' ),
				};
			case 'marketplace':
				if ( purchase.product_type === 'marketplace_theme' ) {
					return {
						primary: showContinueRemoval ? __( 'Continue removal' ) : __( 'Remove theme' ),
						secondary: __( 'Keep theme' ),
					};
				}
				return {
					primary: showContinueRemoval ? __( 'Continue removal' ) : __( 'Remove plugin' ),
					secondary: __( 'Keep plugin' ),
				};
			default:
				return {
					primary: showContinueRemoval ? __( 'Continue removal' ) : __( 'Remove subscription' ),
	...
	// fires on the confirmation click itself, so the button is the action — not a
	// "continue to survey" step.
	return {
