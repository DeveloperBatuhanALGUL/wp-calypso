...
function CancelPurchaseInner() {
	...
	// When the survey was previously completed for this purchase, the confirmation
	// click becomes the action: fire the mutation and navigate, with no survey.
	const skipSurvey =
		config.isEnabled( 'purchases/split-cancel-remove' ) && userHasCompletedCancelSurveyForPurchase;
	...
	const onCancellationStart = (
		...
		// Under the split flag, cancel intent fires the auto-renew mutation immediately
		// on the confirmation click so the subscription is cancelled before the user
		// ever sees the survey. The survey becomes optional and post-facto.
		// Remove intent (CANCEL_WITH_REFUND / REMOVE) is unaffected — mutation still
		// fires at the end of the survey.
		if ( isSplitEnabled && mutationFlowType === CANCEL_FLOW_TYPE.CANCEL_AUTORENEW ) {
			setState( ( state ) => ( { ...state, isLoading: true } ) );
			setPurchaseAutoRenewMutation.mutate(
				{ purchaseId: purchase.ID, autoRenew: false },
				{
					onSuccess: () => {
						...
						// Survey already completed for this purchase — skip it, go straight to purchase settings.
						if ( userHasCompletedCancelSurveyForPurchase ) {
							navigate( {
		...
		// Under the split flag, a user who has already taken the cancellation survey
		// for this purchase should never be asked to take it again. Fire the remove
		// mutation immediately and navigate to the active upgrades list.
		if (
			isSplitEnabled &&
			userHasCompletedCancelSurveyForPurchase &&
			( mutationFlowType === CANCEL_FLOW_TYPE.REMOVE ||
				mutationFlowType === CANCEL_FLOW_TYPE.CANCEL_WITH_REFUND )
		) {
			setState( ( state ) => ( { ...state, isLoading: true } ) );
			recordTracksEvent( 'calypso_purchases_cancel_form_start', {
				cancellation_flow: flowType,
			...
			// eslint-disable-next-line @typescript-eslint/no-use-before-define -- Defined later in the same component; closure is invoked after render.
			fireRemoveMutationAndLeave( mutationFlowType, state.cancelBundledDomain );
			return;
		}
	...
	// Navigate to the Active Upgrades list and fire a remove/cancel-and-refund
	// mutation in the background. The component unmounts before the mutation's
	// built-in onSuccess can trigger a prefix-matched invalidation of
	// purchaseQuery(id), which would 404 for the just-deleted purchase and
	// trap the component in an error state.
	const fireRemoveMutationAndLeave = (
		...
		const showSuccessSnack = () => {
			...
			if (
				...
				createSuccessNotice(
					__(
						'Your plan has been removed. Download a backup to save your content, themes, and plugins.'
					),
					{
						type: 'snackbar',
						actions: [
							{
								...
								url: `https://${ purchase.domain }/wp-admin/export.php`,
							},
						],
					}
				);
		...
		const handleError = async ( error: Error ) => {
			// The proxy can 504 while the backend finishes the delete. Verify
			// whether the purchase actually still exists before surfacing an
			...
			// show an error. The already-shown success snack is transient and
			// will fade; the error snack replaces it in user attention.
			queryClient.invalidateQueries( userPurchasesQuery() );
			createErrorNotice(
				error?.message ??
					sprintf(
						/* translators: %(purchaseName)s is the name of the product. */
						__(
		...
		// Guard: re-strip the purchase from cache whenever any background
		// refetch (route loader prefetchQuery, useQuery refetchOnMount,
		// window focus, mutation onSuccess) returns stale server data that
		// still includes the just-deleted purchase. Server eventual
		// consistency means the purchase stays in API responses for a few
		// seconds after deletion.
							...
							if (
								...
								createSuccessNotice(
									sprintf(
										/* translators: %(productNoun)s is plan/domain/email/theme/plugin/subscription. */
										__( 'Your %(productNoun)s has been removed.' ),
										{ productNoun: getProductNoun( purchase ) }
									),
									{ type: 'snackbar' }
								);
							}
	...
	const onSurveyComplete = async () => {
		// When the split flag is on and the cancel mutation was already fired on the
		// confirmation screen click, skip the mutation here. Survey data was (or will
		// be) submitted by onSubmit; we just need to navigate back to purchase settings
		// where the transient success notice replaces the expiring notice. Keep the
		// survey button busy during the route transition + loader refetch so the
		// handoff feels like a single action rather than two.
		if ( state.mutationFired ) {
			setState( ( state ) => ( { ...state, isLoading: true } ) );
			navigate( {
				to: purchaseSettingsRoute.fullPath,
				params: { purchaseId: purchase.ID },
		...
		if ( displayVariant === 'remove' ) {
			fireRemoveMutationAndLeave( effectiveFlowType, state.cancelBundledDomain );
			return;
		}
	...
	useEffect( () => {
		...
		if (
			state.isLoading &&
			! isDataLoading &&
			! setPurchaseAutoRenewMutation.isPending &&
			! cancelAndRefundMutation.isPending &&
			! removePurchaseMutator.isPending &&
			! extendWithFreeMonthMutation.isPending
		) {
			setState( ( state ) => ( {
				...state,
				...
				<PageHeader
					title={
						<CancelHeaderTitle
							...
							purchase={ purchase }
							surveyStep={ state.surveyStep }
							mutationFired={ state.mutationFired }
						/>
					}
					prefix={ <Breadcrumbs length={ 4 } /> }
