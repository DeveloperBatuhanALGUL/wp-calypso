...
export const DEFAULT_CONTEXT_VALUE: DomainSearchContextType = {
	...
	queries: {
		...
		promotedDomainSuggestions: ( query: string ) => domainSuggestionsQuery( query ),
	...
	return useMemo( () => {
		...
		return {
			...
			queries: {
				...
				promotedDomainSuggestions: ( query ) => ( {
					...domainSuggestionsQuery( query, {
						quantity: Math.max( normalizedConfig.promotedTlds.length * 2, 10 ),
						vendor: normalizedConfig.vendor,
						tlds: normalizedConfig.promotedTlds,
						site_slug: currentSiteUrl,
					} ),
					enabled: false,
					staleTime: Infinity,
